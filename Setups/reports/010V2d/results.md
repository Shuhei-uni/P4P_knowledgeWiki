...
## 9. Comparison: previous checkpoint vs. 5,000 iterations

The previous record above is the 2026-07-22 server-3 checkpoint whose residual export ended at iteration 1520. This is a same-setup diagnostic comparison, but it is not a formal controlled convergence study: the live sessions expose no case/data filenames, DPM track summaries are recomputed at each checkpoint, and final-state-only EWF values do not supply a time-integrated balance.

| Carrier / numerical indicator | Previous (through 1520) | At 5000 | Change |
|---|---:|---:|---:|
| Continuity residual | 1.827e-3 | 8.209e-3 | +349% |
| x / y / z velocity residual | 3.197e-5 / 3.037e-5 / 3.533e-5 | 1.123e-4 / 1.157e-4 / 1.275e-4 | +251% / +281% / +261% |
| k / epsilon residual | 4.295e-2 / 1.181e-1 | 1.133e-1 / 2.358e-1 | +164% / +100% |
| phase-2 volume-fraction residual | 1.372e-3 | 1.401e-3 | +2.1% |
| steam-outlet vapor flow | 81.421242 kg/s | 81.422408 kg/s | +0.001166 kg/s |
| selected-surface imbalance | 110.342758 kg/s | 110.341592 kg/s | -0.001166 kg/s; still 57.54% of inlet |

The flux values are effectively unchanged at the printed scale, whereas the residual record is higher at 5,000 iterations. Without a supplied residual acceptance criterion or flatness window, this is evidence against claiming numerical convergence from either checkpoint.

| EWF final-state quantity on `wall` | Previous | At 5000 | Change |
|---|---:|---:|---:|
| Maximum film CFL | 3.2061953e-3 | 5.0655068e-3 | +58.0% |
| Film inventory | 5.66845e-2 kg | 2.0221152e-1 kg | +256.7% (3.57×) |
| Maximum film thickness | 1.2496226e-4 m | 4.5730619e-4 m | +266.0% (3.66×) |
| Area-weighted thickness | 9.6016164e-7 m | 3.4252e-6 m | +256.7% (3.57×) |
| Derived area-weighted film speed | 5.33223e-2 m/s | 1.3310442e-1 m/s | +149.6% |
| Boundary-film-flow net | 0 kg/s | 0 kg/s | unchanged at printed precision |

Both snapshots retain a low CFL and zero selected-boundary film-flow read, but the film inventory and thickness have grown substantially. This is a final-state observation only; it does not establish accumulation rate or closure.

| Diameter (µm) | Previous final absorbed → at 5000 | Previous escaped → at 5000 | Interaction-event change | Comparison note |
|---:|---:|---:|---|---|
| 5.63 | 0 → 7.007e-5 kg/s | 0.037930 → 0.037790 kg/s | absorbed events not printed → 4 | Fine class remains escape-dominated. |
| 28.14 | 0 → 3.020e-3 kg/s | 0.155800 → 0.151800 kg/s | absorbed events not printed → 42 | Small absorbed and trapped shares now appear. |
| 56.27 | 1.189e-2 → 3.649e-2 kg/s | 0.181400 → 0.155400 kg/s | absorbed events 133 → 408 | Absorption increases by about 3.1×. |
| 112.54 | 0.112000 → 0.176300 kg/s | 0.275100 → 0.209300 kg/s | absorbed 623 → 1052; splash not printed → 128 | Greater final absorption, lower escape. |
| 168.81 | 0.196000 → 0.246300 kg/s | 0.189700 → 0.136900 kg/s | absorbed 1090 → 1427; splash not printed → 152 | Greater final absorption, lower escape. |
| 348.88 | 3.601000 → 4.310000 kg/s | 1.004000 → 0.316600 kg/s | absorbed 1706 → 2194; splash 20 → 256; separation 120 → 179; stripping not printed → 11 | Coarsest class is markedly more absorption-dominated at the later checkpoint. |

**Comparison interpretation:** the later checkpoint shifts the observed DPM fate pattern from steam-outlet escape toward final wall-film absorption in every class that previously had measurable absorption, most strongly for 348.88 µm. Event counts are interaction diagnostics, not additional mass sinks, and changed summary net flows or track counts must not be interpreted as a physical source without a controlled re-run and history-based mass balance.
