...
| Parent setup | [08b — Purnanto parity split-inlet rebuild](../past/reported/08b-purnanto-parity-split-inlet-rebuild.md), unless the operator explicitly selects a different frozen carrier baseline |
...
The current exact reference is the `08b` live checkpoint with `1,309,312` nodes and `7,601,261` tetrahedral cells (`Observed`; see [08b results](../reports/08b/results.md)). It is retained as an additional fine-reference comparison, not assumed to be converged.
