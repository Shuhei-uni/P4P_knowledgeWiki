...
| `09c` | two-way DPM coupling branch | `09c-dpm-ewf-wall-film-reentrainment.md` | `09c-dpm-ewf-wall-film-reentrainment.md` | High | active coupling branch | child of `09`; current active experiment |
...
| `10a` | EWF deposition/drainage child | `active/10a-ewf-deposition.md` | `10a-ewf-deposition.md` | High | active intended control; clean case pending | parent `09c`; no-splash EWF control definition, but the available artifact was read back with wall splash enabled and is reported as a `10a-splash`-type diagnostic |
| `10a-splash` | initialized EWF splash child | `active/10a-splash-ewf-deposition.md` | `10a-splash-ewf-deposition.md` | High | active partial diagnostic; preliminary results available | child of `09c` fallback; EWF DPM coupling and Stanton–Rutland particle splashing enabled; available artifact is labelled `10a` and has `4` splashed particles |
