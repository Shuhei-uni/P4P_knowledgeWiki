...
## Resolved constant-level pressure-outlet qualification pattern

Project evidence from Fluent 2024 R2 supports this reusable order for a VOF
separator with a submerged brine outlet: relax and checksum a liquid-sealed
pool; establish a zero-feed pressure-response bracket from identical cold
loads; introduce a very small common phase-feed fraction; tune only brine
pressure until liquid feed equals emergent liquid drainage; and increase load
with one-factor brackets while gating residuals, Courant, VOF, pressure,
velocity, inventory/storage, clock and phase routing.

Setup 07n achieved exact phase/total closure at 0.05% feed while the pool kept
vapor out of the brine outlet. The startup residual limit lay between 0.05625%
pass and 0.0625% fail. This is method evidence, not a plant boundary: measured
downstream head/resistance and operating level are still required. Fluent
24.2 exposed writable phasic mass-flow-outlet rates but no single bulk mixture
rate, so that outlet type would prescribe routing and was rejected here.

A later 70-step check adds an important reusable caution. One calibrated
pressure step held liquid balance within `0.00171 kg/s` over an earlier
ten-step window while continuity stayed below `0.001`, but the same fixed
pressure later drifted to `0.01155 kg/s` liquid imbalance with the liquid seal
still perfect. Therefore a momentary or short-window mass balance must not be
called constant-level operation. For pressure-controlled drainage, require a
long final balance window and identify the delayed response before each new
held pressure action. Residual convergence, VOF bounds and zero phase leakage
are necessary gates but do not establish hydraulic stationarity. A controller
based only on outlet-flow error is a mass-flow-balance controller; it becomes a
level controller only after a precise interface/inventory observable and a
target level are independently qualified.
