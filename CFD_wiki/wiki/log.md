...
## [2026-08-27] model-method | Separate short pressure balance from sustained level control

- A VOF pressure outlet can show excellent residuals, phase routing and mass
  balance over a short startup window yet drift to substantial over-drainage
  later. Short balance is therefore not evidence of a constant-level solution.
- Per-step proportional outlet-pressure updates can be destabilized by the
  delayed hydraulic response: the higher-gain test crossed and overshot mass
  balance, while the fivefold lower gain corrected too slowly over the observed
  window.
- Reusable rule: qualify controller response time before tuning gain; use a
  settle-and-update or filtered level/flow signal, require sustained phase and
  storage closure, and keep CFD-derived outlet pressure distinct from plant
  head/resistance data.
- Project evidence remains diagnostic: DPM/EWF/sources stayed off, the liquid
  steam seal remained intact, and no efficiency or plant-valid pressure claim
  follows.

## [2026-08-25] model-method | Preserve VOF pathline-video interpretation limits

- Live Fluent 2024 R2 post-processing verified that `results.graphics.pathline`
  can export massless carrier-flow pathway frames while DPM remains zero/off.
- A release surface must be read back exactly. The 0.05%-feed endpoint accepted
  `liquidinlet` but substituted `wall-fluid` for `steaminlet`; the ambiguous
  steam-origin scene was excluded.
- Pathline range choices in this case were `auto-range` and `clip-to-range`,
  not the contour-style `auto-range-off` hierarchy. Exact readbacks and raw
  PNGs are retained with the video manifest.
- Reusable interpretation rule: a frozen VOF pathline uses the shared carrier
  velocity and is neither a phase-specific material history nor DPM evidence.
  Keep mass-flux phase routing and carryover claims separate.

## [2026-08-25] model-method | Qualify resolved VOF drainage at low feed

- Files updated: separator-efficiency synthesis, CFD log and Fluent settings
  successful-path record.
- Reusable evidence: a submerged resolved liquid pool can block vapor while a
  pressure outlet drains liquid with converged residuals and closed phase/total
  balance. Use identical cold loads, a zero-feed pressure-response bracket and
  one-factor inlet-load steps.
- Fluent 2024 R2 caveat: the VOF mass-flow-outlet object exposed phasic rates
  but no writable single bulk mixture rate; forcing phasic values would
  predetermine routing.
- Evidence limit: the demonstrated 0.05% feed and CFD-interpolated pressure are
  numerical diagnostics, not plant validation. The residual load boundary was
  0.05625% pass versus 0.0625% fail.

## [2026-08-07] model-method | Record diagnostic constant-water-level liquid sink
- Files created/updated:
  - `wiki/synthesis/fluent-separator-efficiency-methods.md`
  - `wiki/log.md`
- Reason: the split-inlet project branch demonstrated liquid accumulation in a closed-bottom steady model and implemented a liquid-only bottom-adjacent cell source as an unresolved constant-level reservoir test.
- Reusable rule: attach mass removal to the liquid phase, remove carried mixture momentum, leave vapor untouched, include the integrated sink in liquid balance, and qualify from a clean freshly initialized mesh.
- Evidence limit: this source abstraction is not a resolved brine outlet and cannot support separator-efficiency or validation claims until its steady behavior and parameter/layer sensitivity pass.
...
## [2026-08-15] query | Add multiphase gravity pressure-outlet guidance

- Files created/updated:
  - `wiki/guidance/fluent-general-click-by-click.md`
  - `wiki/log.md`
- Reason: preserve the documented Fluent pressure convention and the initialization/backflow checks needed when a liquid outlet is vertically separated from a vapor outlet.
- Notable assumptions introduced or removed:
  - Removed manual hydrostatic-offset tuning as a default step when gravity is active; clarified that backflow phase fraction acts only during reverse flow and that initialization-dependent liquid reservoirs should trigger a transient test when no unique steady state is obtained.

## [2026-08-21] model-update | Record hydrostatic-rest and DPM-cleanup evidence

- Files created/updated:
  - `../../Setup report/07l-split-inlet-hydrostatic-rest-isolation.md`
  - `../../PyAnsys/scripts/setup/prepare_setup07l_hydrostatic_rest.py`
  - `../../PyAnsys/scripts/setup/run_setup07l_hydrostatic_rest.py`
  - project status, validation, blockers, experiments and logs
- Reason: separate Fluent connection symptoms from numerical failure and test
  the resolved brine mesh/pool without through-flow or inherited DPM parcels.
- Notable assumptions introduced or removed:
  - Removed the assumption that DPM interaction-off guarantees no parcel
    tracking; six inherited injections must be deleted explicitly.
  - Removed the assumption that equal pressure at vertically separated vapor
    and submerged liquid faces is neutral. The closed-pool result developed
    `2090.4 Pa` lower-face head, close to `rho_l g h`.
  - Retained the rule that a pressure bracket derived from CFD is diagnostic,
    not plant validation, until downstream pressure/resistance/level data exist.
## [2026-08-21] model-update | Add resolved-brine pressure-opening evidence

- Files created/updated: setup-07m report, PyAnsys pressure-opening/ramp
  controllers, project experiments/current status/blockers/validation and logs.
- Reason: test the documented gravity/modified-pressure outlet treatment from
  an accepted hydrostatic-rest field before reintroducing carrier feed.
- Notable evidence: three diagnostic pressures produced a monotonic sign
  bracket after ten transient steps; a guarded 0.1% inlet hold reached
  continuity `0.00186867` without DPM, EWF or sink sources.
- Limitation retained: the CFD-derived `1.12209 MPa` pressure is not downstream
  plant validation and cannot replace a measured pressure or resistance curve.

## [2026-08-23] model-update | Complete closed-pool VOF and coupling sensitivities

- Files created/updated: setup-07n report, guarded PyFluent solver-screen
  scripts, machine-readable matched-window comparisons, and project status,
  experiments, blockers, validation and logs.
- Reason: distinguish outlet/control-model uncertainty from timestep,
  inner-iteration, VOF-formulation, interface-scheme and pressure-velocity
  coupling effects before attempting constant-level drainage.
- Notable evidence: explicit timestep halving did not become asymptotic;
  increasing inner iterations changed the field by less than `0.000009%`;
  second-order implicit Modified-HRIC retained a `6.831%` maximum-velocity
  difference from explicit; explicit plain Coupled reproduced PISO within
  `0.000063%` at `3.234x` the summed solve-step wall time.
- Reusable interpretation: residual improvement alone does not select a VOF
  formulation or pressure-velocity solver. Retain PISO for efficient bounded
  diagnostics here, but do not call the closed pool an open-drain or constant-
  level solution. Downstream head/resistance/level data remain the primary
  physical closure requirement.

## [2026-08-25] query | Qualify resolved constant-level drainage methods

- Files updated: separator-efficiency synthesis and Fluent general guidance.
- Reason: translate the constant-water-level assumption into a staged resolved
  VOF drainage method that retains an emergent liquid steam seal.
- Reusable finding: establish a zero-feed pressure-response sign first, then
  test a bulk balanced-rate outlet, then bounded pressure feedback. Prescribing
  phasic outlet rates does not test phase routing, and exact rate-forced balance
  is not plant validation.
- Evidence boundary: published VOF work supports matching total outlet and
  inlet mass flow to maintain a level, while Fluent documents pressure and
  outward mass-flow outlets. The geothermal baseline itself excluded resolved
  water flow into the brine pipe. Plant pressure, resistance and operating
  level therefore remain required closure data.

## [2026-08-28] query | Require sustained balance for pressure-controlled drainage

- Files updated:
  - `wiki/synthesis/fluent-separator-efficiency-methods.md`
  - `wiki/log.md`
- Reason: incorporate the completed 70-step resolved-drainage evidence into a
  reusable qualification rule for pressure-controlled VOF separators.
- Notable assumptions introduced or removed:
  - Removed the assumption that one short balanced window plus low residuals
    is sufficient evidence of constant-level operation.
  - Added the rule that outlet-flow feedback is only mass-flow-balance control
    until a precise level observable and target level are qualified; require a
    long final balance window and delayed-response identification.
