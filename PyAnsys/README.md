...
- `scripts/connection/check_setup07b_sink_status.py`: read-only local/live status for the setup-07b qualification
...
- `scripts/setup/setup07b_constant_water_level_sink.py`: compile, attach, read back, and checkpoint the setup-07b liquid-only sink
- `scripts/setup/prepare_setup07b_clean_900k.py`: build the accepted setup-07b start state from the clean original 900k mesh with no saved solution data
- `scripts/setup/run_setup07b_sink_qualification.py`: ramp and qualify `tau=0.1 s` with source-inclusive balances and monitor-based stopping
- `scripts/inspection/verify_constant_water_level_sink.py`: cold-reload and verify source hooks, UDMs, RP controls, DPM-off state, and baseline parity
