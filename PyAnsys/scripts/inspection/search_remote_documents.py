...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--list-all",
        action="store_true",
        help="List every file directly under root without extension filtering or recursion.",
    )
    parser.add_argument(
        "--list-directories",
        action="store_true",
        help="List directories directly under root.",
    )
...
def main() -> int:
    ...
    if args.list_directories:
        command = (
            ...
            f"\"Get-ChildItem -Path '{root}\\*' -Directory -ErrorAction SilentlyContinue"
        ...
        command = (
            "powershell -NoProfile -ExecutionPolicy Bypass -Command "
            f"\"Get-ChildItem -Path '{root}\\*' -File -ErrorAction SilentlyContinue"
            f"{pattern_filter} | Sort-Object FullName | ForEach-Object {{ "
