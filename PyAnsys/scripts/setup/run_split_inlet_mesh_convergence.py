...
def apply_settings(solver: Any, path: str) -> str:
    ...
    attempts: tuple[tuple[str, Callable[[], Any]], ...] = (
        # The generated Settings API is the path that produced every accepted
        # mesh-study import transcript. A fresh Fluent session normally
        # returns in seconds; the outer guarded controller bounds a wedged RPC.
        ...
        # Keep TUI as the final version/path fallback. Some 2024 R2 wrappers
        # accept this call but perform no import, so output evidence is required
        # before any attempt is treated as successful.
    ...
    for label, call in attempts:
        ...
        if not rejected and applied:
            return text
        if not applied:
            text += "\nsettings import rejected: required application markers were absent\n"
