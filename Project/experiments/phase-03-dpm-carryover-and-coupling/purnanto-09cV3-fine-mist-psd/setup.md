...
| Linked report | [09cV3 fine-mist allocation diagnostic results](results.md) |
...
Use the same geometry and selected mesh checkpoint as the copied `09cV2` parent. Mesh independence is unresolved, so any `09cV3` comparison is conditional on the exact mesh/checkpoint being recorded; do not attribute a difference to PSD if the mesh, carrier maturity, or checkpoint changes at the same time ([meeting report](../../../technical/mesh-conversion-dpm-mass-sensitivity-meeting-report.md), §§1 and 3).
...
| DPM size range | `5–100 µm` | **Assumed** provisional fine-mist representation ([fine-mist decision](fine-mist-interpretation.md), §§3–6) |
...
- retain `diagnostic only` if the inherited carrier imbalance, residual drift, or DPM source response remains unresolved. The meeting record explicitly identifies the existing 5% case as diagnostic rather than converged ([meeting report](../../../technical/mesh-conversion-dpm-mass-sensitivity-meeting-report.md), §2).
...
- [09cV2 diagnostic results](../purnanto-09cV2-dpm-partition-control/results.md)
...
| Verification | strict explicit-reload readback (historical machine artifact path: `../../../PyAnsys/output/09cV3_server2_10pct_verification_20260807.json`; not migrated) |

The seven-bin flow values are the project’s tabulated `10%` values from the [fine-mist allocation sweep](fine-mist-interpretation.md#8-scaling-across-the-dpm-fraction-sweep); they sum to `11.692000 kg/s`. The planned `5,000`-iteration run must still be reported separately from this case-only verification, including its actual completed iteration count and any numerical/DPM monitoring evidence.
...
| Verification | strict explicit-reload readback (historical machine artifact path: `../../../PyAnsys/output/09cV3_mass_flow_02pct_from_05pct_20260807.json`; not migrated) |

The seven surface injections remain on `steaminlet`, retain the documented diameters and all inherited non-loading settings, and use the tabulated 2% flow scaling in the [fine-mist allocation sweep](fine-mist-interpretation.md#8-scaling-across-the-dpm-fraction-sweep). A later explicit reload of the paired `...-iter5000.cas.h5`/`.dat.h5` checkpoint established an available 5,000-iteration result checkpoint and produced the scoped phase-flux and DPM terminal-fate reports in the [09cV3 results record](results.md). The result remains diagnostic because carrier closure, residual maturity, and the substantial incomplete-DPM fraction are unresolved.
...
The strict read-back snapshot is 09cV3_student_finemist_verification_20260804.json (historical machine artifact path: `../../../PyAnsys/output/09cV3_student_finemist_verification_20260804.json`; not migrated), and the case-only builder is [build_09cV3_student_finemist_from_09cV2.py](../../../../PyAnsys/scripts/setup/build_09cV3_student_finemist_from_09cV2.py).
...
The stopped-run record is 09cV3_student_50_then_100_run_20260804.json (historical machine artifact path: `../../../PyAnsys/output/09cV3_student_50_then_100_run_20260804.json`; not migrated), the run-state record is 09cV3_student_50_then_100_run_state_20260804.json (historical machine artifact path: `../../../PyAnsys/output/09cV3_student_50_then_100_run_state_20260804.json`; not migrated), and the reusable runner is run_09cV3_student_50_then_100.py (historical machine artifact path: `../../../PyAnsys/scripts/setup/run_09cV3_student_50_then_100.py`; not migrated).
...
| Verification | strict server-1 readback (historical machine artifact path: `../../../PyAnsys/output/09cV3_mass_flow_verification_20260804.json`; not migrated) |
...
| Local worker state | 09cV3_student_5000_resilient_state_20260804.json (historical machine artifact path: `../../../PyAnsys/output/09cV3_student_5000_resilient_state_20260804.json`; not migrated) |
| Local worker log | 09cV3_student_5000_resilient_20260804.log (historical machine artifact path: `../../../PyAnsys/output/09cV3_student_5000_resilient_20260804.log`; not migrated) |
| Worker | run_09cV3_student_5000_resilient.py (historical machine artifact path: `../../../PyAnsys/scripts/setup/run_09cV3_student_5000_resilient.py`; not migrated) |
