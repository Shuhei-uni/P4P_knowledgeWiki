> **Retired source:** Setups/full-geometry/mixture/steady-liquid-outlet/03a-08b-parity-full-geometry-baseline.md
...
> **Child setup:** [`03B` historical continuation plan](../index.md) would continue only from a qualified 03A field while changing brine-outlet pressure; no separately retained 03B record exists.
...
If one Stage-2 route creates a stable developed field and that improvement survives restoration of the canonical 08b/RNG/second-order settings, save an immutable qualified 03A case/data checkpoint and begin the historical `03B` brine-pressure continuation plan from that exact field without reinitialization; the current retained set is summarized in the [Phase 5 index](../index.md).
