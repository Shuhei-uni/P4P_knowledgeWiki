# P71A-T1-REALIZABLE-KEPSILON — results

## Answer at a glance

- **Question:** Does changing only the k-epsilon closure from RNG to realizable
  k-epsilon alter the finite-horizon turbulence and absorber-coupled numerical
  signature?
- **Result:** The exact-parent child completed the declared 500 active
  iterations. The realizable branch produced a distinct, nonstationary
  finite-horizon trajectory with persistent reverse flow and broad turbulent-
  viscosity ratio limiting. In the late native-iteration window 1250–1500,
  total liquid mass rose from 1,010.19 to 1,158.07 kg and continuity ended at
  `3.85e-1`.
...
- **Decision / gate status:** `COMPLETE_VERIFIED` for T1-realizable discovery
...
viscous closure from RNG to realizable k-epsilon. The restart field was not
...
The realizable closure readback passed with Fluent 2025 R2's branch-specific
representation: the RNG/Kato-Launder option is absent from the realizable
branch, while `rke_cmu_rotation_term` read back `false`; the absent option was
therefore treated as not applicable, not silently assumed. The prepared
save/reopen, smoke test, checkpoint pair, and final active-500 pair all passed.
The integrated phase-2 source readback was `-116.9200000000002 kg/s` over
`p7-e5-lower-y010`, matching the declared command within readback precision.

A first preflight attempt was safely blocked before solving because the initial
validator did not yet encode this Fluent branch-specific absence. The
live-tree inspection and corrected readback rule are preserved under
[`attempts/20260911T100015Z`](attempts/20260911T100015Z); the successful retry
was loaded from a fresh exact parent and is the result reported here.

## Core visual evidence

![F1 — turbulence stability history](figures/F1-turbulence-stability-history.png)

_Figure F1. Native residual histories for the finite realizable k-epsilon child
...
- **Observed:** continuity decreases through much of the late window but turns
  upward again near the end, finishing at approximately `3.85e-1`; the tail is
  not a stationary low-residual state.
...
- **Observed:** late phase-2 boundary net without the absorber term averages
  about `115.86 kg/s` with a `0.84 kg/s` range, while the integrated source
...
- **Observed:** total liquid mass rises by about `147.89 kg` over the late
...
- The limiter reached approximately 137,000 cells at the end of the solve.
  This is a recorded numerical diagnostic, not proof of a physical mechanism.
- Fluent emitted the inherited load-time `CDR invalid argument` / partial-grid
  warning during reopen before successfully reading the saved pair; the final
  readback, report recovery, and runtime validation all passed.

## Interpretation and claim boundary

- **Observed:** the realizable closure can advance from the exact parent and
  produces a distinct finite-horizon residual/inventory trajectory while
  retaining strong viscosity limiting and reverse flow.
- **Inferred:** realizable k-epsilon changes the numerical trajectory, but the
  finite screen does not establish a stationary or qualified branch.
- **Competing explanation:** residual and inventory differences may reflect the
  coupled closure/absorber/phase-routing response; this child isolates only the
...
T1-realizable is complete as the final approved discovery child. Compare it
with T0 and T1-standard using the family comparison artifacts. The finite
screen remains discovery evidence only: no T2–T4 or hypothesis route is
authorized by this result.

## Run and artifact details

- Run ID: `P71A-T1-REALIZABLE-KEPSILON-student-20260911T100157Z`
- Remote final pair: `C:\Users\Shuhei Yokkaichi\Documents\FluentRuns\Phase07\Phase71A\TurbulenceFamily\P71A-T1-REALIZABLE-KEPSILON\20260911T100157Z\P71A-T1-REALIZABLE-KEPSILON-active500.{cas.h5,dat.h5}`
