...
- purpose: execute the user's requested `1,000`-iteration 03A carrier checkpoint from the verified case-only artifact using Fluent-native initialization/iteration and gRPC-only Student access.
- what changed: Fluent read the 03A case, completed Hybrid Initialization, ran exactly `1,000` steady iterations, wrote the paired endpoint, exported residuals, and left the Fluent session open. The endpoint was then explicitly reloaded through gRPC and passed case/data, flux, and residual artifact checks.
- numerical result: final continuity was `1.6043e-1`; final `vf-phase-2` was `6.5142e-3`; velocity residuals were approximately `1.5–1.7e-4`; reverse flow remained on `334` pressure-outlet faces; turbulent-viscosity limiting occurred. Flux extraction gave liquid inlet `116.8468 kg/s`, vapour inlet `81.6395 kg/s`, liquid to brine `82.6125 kg/s`, liquid to steam `0.1425 kg/s`, vapour to brine `37.5263 kg/s`, and vapour to steam `44.1293 kg/s`. Including both pressure outlets, outlet magnitude was `164.4105 kg/s` versus `198.4863 kg/s` inlet, leaving a `34.0758 kg/s` (`17.17%`) diagnostic balance residual.
- post-processing correction: the generic flux helper previously used a simplified-Purnanto fallback that omitted a physical brine outlet from the fallback mixture balance. It now detects a second discovered pressure outlet, reports total phase outflow across all pressure outlets, and marks the balance diagnostic-only for this case.
- outcome and next action: `RUN_COMPLETED_ENDPOINT_VERIFIED; NOT_CONVERGED`. This is not an accepted 03A parent and must not seed 03B. Diagnose the non-zero balance/reverse-flow/viscosity-limiting state and resolve the remaining operating-temperature, Mixture-interaction, and geometry-preflight gates before any continuation.

## [2026-08-17] setup-update | Create 03A 08b-parity full-geometry case-only artifact

- files created/updated: `../../PyAnsys/scripts/setup/build_03a_08b_parity_full_geometry.py`, `../../PyAnsys/output/03a_08b_parity_full_geometry_build_20260817T103746Z.json`, `wiki/progress/current-status.md`, `wiki/progress/experiments.md`, `wiki/log.md`, `../../PyAnsys/knowledge/fluent-settings/logs/successful_paths.md`
- purpose: trace the requested 08b parity setup back to the audited Purnanto/00a carrier definition and current split-inlet evidence, then materialize the 03A full-geometry case on Student without running it.
- what changed: the exact `Full-geomV2-231kcells.msh.h5` mesh was loaded on `student` through Fluent gRPC; the builder applied the current setup's pure-phase split Velocity Inlets, brine pressure outlet, audited psep materials, steady Mixture/RNG k-epsilon solver stack, and documented numerics/under-relaxation controls. The case was written and reloaded by full remote path as `C:\Users\Shuhei Yokkaichi\Documents\CFD\Brine outlet\03A-08b-parity-full-geometry-steady-preinit-20260817T103746Z.cas.h5`, and gRPC `remote_file_exists` readback confirmed the case exists without a matching data file.
- lineage decision: no separate current 08b-named case existed in the accessible Student spool. The historical 08b saved-case description uses Mass-Flow Inlets, while the active 03A setup explicitly requires the split pure-phase Velocity Inlet representation; the current setup instruction wins for this case. The excluded `FG-MIX-T01-S1-C1375` branch and later DPM payload were not used.
- readback: `231,376` cells, `697,078` nodes, `1,096,333` faces; inlet areas `0.0048896664` and `0.51928634 m2`; outlet areas `0.60094806` and `0.20143996 m2`; minimum orthogonal quality `0.200006`; maximum aspect ratio `82.6482`; full-path reload contract matched.
- assumptions introduced or retired: no guessed phase-interaction values were written. Student's operating-temperature and Mixture interaction branches were inactive, and the outlet perimeter/hydraulic-diameter preflight remains open; provisional Dh values are explicitly not geometry evidence. No initialization, patch, iteration, solve, DPM/EWF activation, or data-file write occurred.
- next immediate action: complete native preflight readback of outlet wetted perimeters/hydraulic diameters and any supported Mixture interaction settings, then decide whether the setup's run gate can be closed.
...
## [2026-08-16] model-update | Complete 02e Stage-2 Y010 brine-outlet characterization

- files created/updated: `../../Setups/reports/02e/stage2-results-20260816.md`, `../../Setups/active/02e-mixture-y010-brine-outlet-boundary-characterization.md` was used as the governing setup, `../../../PyAnsys/scripts/setup/build_02e_stage2_campaign.py`, `../../../PyAnsys/scripts/setup/run_02e_stage2_case.py`, `../../../PyAnsys/scripts/inspection/analyze_02e_stage2_histories.py`, `../../../PyAnsys/output/02e_stage2_build_20260816.json`, four native Stage-2 journals/manifests, and the recovered Stage-2 history/transcript bundle under `../../../PyAnsys/output/02e_stage2_recovered_20260816/`.
- purpose: build and run the user-selected PO/OV Stage-2 matrix from the unchanged initialized Y010 parent for native 500-iteration attempts and record the same inventory/phase-flux evidence as the Stage-1 results report, plus the newly required total-domain liquid-volume monitor.
- what changed since last update: all four children were independently built and submitted as separate native Fluent runs. PO `1.175 MPa` and `1.190 MPa` terminated with FPE at `453` and `415`; OV `K=3` and `K=7` completed `500` and wrote paired endpoints. Offline recovery produced Y010/Y030 inventories, total-domain liquid volume, phase-separated fluxes, residual histories, summary CSV/JSON, and figures.
- measured direction: complete OV cases have final-100 liquid balances `−545.702` and `−480.950 kg/s`; `K=7` lowers measured liquid-to-brine flow relative to `K=3` but neither case establishes a mass-conserving or converged operating point.
- evidence boundary: failed PO outlet fluxes are corrupted after residual blow-up and are not treated as physical endpoints. Complete OV status means the native solve reached iteration 500 and wrote case/data; it does not establish convergence, validation, separator efficiency, or automatic control selection.
- next immediate action: obtain user direction on the next objective before selecting additional PO/OV values; preserve total-domain liquid-volume monitoring and common-window evidence for any follow-up.

## [2026-08-16] model-update | Replace 02c upper sweeps with single Student H point

- files created/updated: `../../Setups/active/02c-mixture-brine-outlet-pressure-sensitivity-unprimed.md`, `../../Setups/reports/full-geometry/mixture/steady-liquid-outlet/02c/results.md`, `../../Setups/reports/full-geometry/mixture/steady-liquid-outlet/02c/future-runs.md`, `wiki/progress/current-status.md`, `wiki/progress/experiments.md`, `wiki/log.md`, `../../../PyAnsys/scripts/setup/build_02c_student_h1140_case.py`, `../../../PyAnsys/scripts/setup/run_02c_student_h1140.py`, and the generated build/journal/post-processing artifacts under `../../../PyAnsys/output/`.
- reason: user requested removal of the planned H20–H50 and I20–I160 pressure sweeps and replacement with one H test at `1.140 MPa` on the Student endpoint.
- setup change: active 02c matrix now retains the existing A–G evidence and adds only `02c-H` at `1.140 MPa`; former H/I sweep members are superseded as future work, with historical preparation notes retained only for traceability.
- execution: explicit Student pre-initialization surrogate was loaded; only brine-outlet gauge pressure changed from `1.125` to `1.140 MPa`; case-only child was reload-verified; Fluent-native Hybrid Initialization plus `500` steady iterations completed and wrote a paired endpoint.
- measured evidence: Student endpoint has `500` residual points; final continuity `2.288839e-1`; raw liquid brine flow is reverse (`+616.795 kg/s`); vapour brine flow is `0 kg/s`; post-reload turbulent-viscosity limiting affected approximately `27,030` cells.
- evidence boundary: H is `Unstable / Indeterminate` and not converged; its Student mesh-derived surrogate lineage is not certified as server-2/production 02c mesh parity, and no pressure-selection or separator-performance claim is made.
- next immediate action: await user direction; do not restart the cancelled H/I sweeps or interpret H as an operating-pressure result without a new scope and lineage decision.

## [2026-08-16] model-update | Complete FG-MIX-T01 Stage-1 exact-mesh pressure-candidate screen
- files created/updated: `../../Setups/full-geometry/mixture/transient-liquid-outlet/stage-01-02-steady-parent-and-transient-start.md`, `../../Setups/reports/full-geometry/mixture/transient-liquid-outlet/stage-01-candidate-screen-20260816.md`, `wiki/progress/current-status.md`, `wiki/progress/experiments.md`, `wiki/log.md`, `../../../PyAnsys/scripts/setup/build_fg_mix_t01_stage1_candidates.py`, `../../../PyAnsys/scripts/setup/run_fg_mix_t01_stage1_candidate.py`, and the corresponding native journals, manifests, endpoint records, and post-processing bundles under `../../../PyAnsys/output/`.
- purpose: execute the user-requested quick Stage-1 three-case screen between the 02c G/H pressure region and identify a provisional candidate parent for the next transient-stage construction.
- setup contract: all three independent case-only children used the unchanged `Full-geomV2-231kcells.msh.h5` mesh (`231,376` cells; `697,078` nodes), the same unpatched Mixture/RNG k-epsilon parent contract, and brine-outlet gauge pressures of `1.1360`, `1.1375`, and `1.1390 MPa`. Fluent owned Hybrid Initialization, the native `1,000` steady iterations, and paired case/data writes.
- measured evidence: all three paired endpoints were verified and reloaded. `C1375` at `1.1375 MPa` had the smoothest retained residual tail overall and the lowest measured vapour brine-outlet fraction (`21.00%`), while `C139` had lower continuity/velocity residual levels but more variable turbulent dissipation and substantially worse phase routing (`28.01%` vapour to brine and higher liquid-to-steam flow). The open-domain liquid balance remains inventory-draining for all three cases.
- selection and evidence boundary: `FG-MIX-T01-S1-C1375` is the provisional candidate parent for the next setup-construction stage because it is the best-balanced numerical/phase-routing point in this short screen. None of the three is converged or a validated separator operating point; the selection remains diagnostic and conditional on the Stage-2 exact-mesh/model/BC readback gate.
- next immediate action: reload the `C1375` paired endpoint, re-verify the exact mesh and inherited setup contract, and use that endpoint only as the candidate parent for the prescribed Stage-2 branches; retain the other two endpoints as comparison evidence.

## [2026-08-16] model-update | Create FG-MIX-T01 Stage-2 transient startup-state pairs
- files created/updated: `../../Setups/full-geometry/mixture/transient-liquid-outlet/stage-01-02-steady-parent-and-transient-start.md`, `../../Setups/reports/full-geometry/mixture/transient-liquid-outlet/stage-02-start-states-20260816.md`, `wiki/progress/current-status.md`, `wiki/progress/experiments.md`, `wiki/progress/blockers.md`, `wiki/log.md`, `../../../PyAnsys/scripts/setup/build_fg_mix_t01_stage2_start_states.py`, and `../../../PyAnsys/output/fg_mix_t01_stage2_start_states_20260816.json`.
- purpose: use the accepted `FG-MIX-T01-S1-C1375` endpoint to construct the matched Stage-2 `INIT-S` developed-field and `INIT-H` Hybrid-Initialization startup states for T-PO-1 at `1.200 MPa` brine-outlet gauge pressure.
- setup contract: both independent pairs preserve `Full-geomV2-231kcells.msh.h5` (`231,376` cells; `697,078` nodes), Mixture/RNG `k-epsilon`, the verified materials/inlets/steam outlet/gravity/operating-pressure state, and the common provisional transient settings: Fluent `unsteady-2nd-order-bounded`, PISO with one neighbor-correction iteration, fixed `2.5e-4 s` timestep, maximum `20` iterations per timestep, and flow time `0 s`.
- construction evidence: both pairs were written and reloaded; both have the approved `33,315`-cell Y010 register and one phase-2 liquid patch. No transient timestep or steady iteration was run.
- gate status: `INIT-S` post-patch Y010 liquid volume is `4.793078931 m³` and `INIT-H` is `4.790652590 m³`, a `0.05065%` difference. Fluent 2025 R2 also refused several optional volume-report file/instantaneous-history writes as read-only. The cases are valid construction artifacts, but the Stage-2 equivalence gate remains open and Stage 3 is held.
- next immediate action: resolve or explicitly accept the Y010 post-patch difference and repair/verify the affected report definitions before submitting any initialization-comparison run.

## [2026-08-16] model-update | Cancel FG-MIX-T01 Stage-3 queue and prepare total-liquid-mass monitoring
- files created/updated: `../../Setups/reports/full-geometry/mixture/transient-liquid-outlet/stage-03-initialization-comparison-20260816.md`, `../../Setups/reports/full-geometry/mixture/transient-liquid-outlet/index.md`, `../../Setups/reports/full-geometry/mixture/transient-liquid-outlet/stage-02-start-states-20260816.md`, `wiki/progress/current-status.md`, `wiki/progress/experiments.md`, `wiki/progress/blockers.md`, and the monitor-preparation script/manifest under `../../../PyAnsys/`.
- reason: the user requested cancellation of the active `1,000`-step transient queue and asked whether total liquid mass could be tracked throughout the run.
- execution: the native solver interrupt succeeded while `INIT-H` was in progress, before `INIT-S` began. The transcript was stopped and Fluent was returned to idle. No paired Stage-3 endpoint was written for either branch.
- evidence boundary: the canceled attempt has no complete physical-time trajectory and no reliable total-liquid-mass history; it is retained as an audit record only and is excluded from comparison.
- monitoring action: saved Stage-2 INIT-S/INIT-H pairs were reloaded without reinitialization, repatching, or advancing. New monitor-ready pairs were written with direct full-domain phase-2 `volume-mass` report `fg_mix_t01_s3_total_liquid_mass`; report-file output was true after reload verification. The exact mesh remained `Full-geomV2-231kcells.msh.h5` with `231,376` cells and `697,078` nodes.
- next immediate action: do not restart automatically; when authorized, submit the same `1,000`-step / `0.25 s` comparison using the monitor-ready pairs and recover the mass report history.

## [2026-08-16] model-update | Run FG-MIX-T01 Stage-3 from total-mass monitor-ready pairs
- files created/updated: `../../Setups/reports/full-geometry/mixture/transient-liquid-outlet/stage-03-initialization-comparison-20260816.md`, `../../Setups/reports/full-geometry/mixture/transient-liquid-outlet/index.md`, `wiki/progress/current-status.md`, `wiki/progress/experiments.md`, and the replacement Stage-3 manifest/journal under `../../../PyAnsys/output/`.
- execution: the correct `PyAnsys/.venv` connected to Student Fluent 2025 R2. Both monitor-ready INIT-H/INIT-S pairs passed reload verification against `Full-geomV2-231kcells.msh.h5` (`231,376` cells; `697,078` nodes). The native journal began INIT-H with `1,000` steps, `2.5e-4 s`, and the direct phase-2 total-liquid-mass report loaded.
- failure: INIT-H diverged and terminated at transient step `53` with a floating-point exception. Continuity reached `3.1443e+57`; AMG divergence was reported, reversed flow appeared at both pressure outlets, and turbulent-viscosity limiting reached all `231,376` cells. INIT-S never began.
- evidence boundary: no paired Stage-3 endpoint was written, and the partial mass-monitor output is not treated as a complete physical history. The run is failed/diagnostic only; no initialization winner or stability conclusion is claimed.
- next immediate action: hold automatic retries and obtain a user decision on stabilization/timestep/initialization changes before another native Stage-3 submission.

## [2026-08-17] model-update | Run FG-MIX-T01 Stage-3 NP-DT1 no-patch control
- files created/updated: `../../Setups/reports/full-geometry/mixture/transient-liquid-outlet/stage-03-no-patch-NP-DT1-20260817.md`, `../../Setups/reports/full-geometry/mixture/transient-liquid-outlet/index.md`, `wiki/progress/current-status.md`, `wiki/progress/experiments.md`, `wiki/progress/blockers.md`, `../../../PyAnsys/scripts/setup/run_fg_mix_t01_stage3_no_patch_dt1.py`, and the NP-DT1 journal/manifest under `../../../PyAnsys/output/`.
- purpose: execute only the first sequential no-patch control from the developed C1375 steady parent, holding the transient formulation fixed and using both pressure outlets at `1.120 MPa` gauge.
- execution: a fresh start pair was written and reload-verified; no Hybrid Initialization and no Y010 patch were applied. Fluent then owned the native `200`-step attempt at `2.5e-4 s` with a `20`-iteration-per-timestep cap.
